...
ponderacion_evaluaciones = [float(input(f"Ponderación de Evaluación {i + 1}: ")) for i in range(cantidad_evaluaciones)]
...
while True:
    ...
    if opcion == "1":
        ...
        estudiante_idx = int(input(f"Ingrese el número del Estudiante (1-{cantidad_alumnos}) para ingresar nota: "))
        estudiante = alumnos[estudiante_idx - 1]

        estudiante["notas"] = [float(input(f"Ingrese la nota de la Evaluación {i + 1}: ")) for i in
                               range(cantidad_evaluaciones)]

        if tiene_examen.lower() == "s":
            estudiante["examen"] = float(input("Ingrese la nota del Examen: "))
        ...
        estudiante_idx = int(
            input(f"Ingrese el número del Estudiante (1-{cantidad_alumnos}) para ingresar asistencia: "))
        estudiante = alumnos[estudiante_idx - 1]

        estudiante["asistencia"] = float(input(f"Ingrese el porcentaje de asistencia (0-100): "))
        ...
        estudiante_idx = int(
            input(f"Ingrese el número del Estudiante (1-{cantidad_alumnos}) para consultar su condición: "))
        estudiante = alumnos[estudiante_idx - 1]
